...
function calcularTotalPagoReal(texto: any) {
  ...
  const cartaoTotal = [...linha.matchAll(/(?:CARTÃO|LINK CARTÃO).*?PAGO\s*R?\$?\s*([\d\.,]+)/g)];
  ...
  const cartaoParcelado = [...linha.matchAll(/(?:CARTÃO|LINK CARTÃO).*?PAGO\s+(\d+)\s*X\s*([\d\.,]+)/g)];
  ...
  const primeira = [...linha.matchAll(/PAGO\s+PRIMEIRA\s+PARCELA\s*R?\$?\s*([\d\.,]+)/g)];
  primeira.forEach((m) => {
    const valor = parseValor(m[1]);
    if (!isNaN(valor)) total += valor;
  });

  const pagoAvulso = [...linha.matchAll(/PAGO(?:\s+A\s+VISTA|\s+VIA\s+PIX|\s+DINHEIRO|\s+DÉBITO)?\s*R?\$?\s*([\d\.,]+)/g)];
...
export default function Relatorios() {
  ...
  const porArea = {
    banc: filtrado.filter((a) => String(a.Curso || "").toUpperCase().includes("BANCÁRIO")).length,
    agro: filtrado.filter((a) => String(a.Curso || "").toUpperCase().includes("AGRO")).length,
    ing: filtrado.filter((a) => String(a.Curso || "").toUpperCase().includes("INGLÊS")).length,
